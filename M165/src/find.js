...
const findDate = db.mietvertrag.find({
  von: { $gt: new Date("2022-01-01") }
}, { "mieter.name": 1, _id: 1 });
print("one")
print(findDate)

const findOr = db.mietvertrag.find({
  $or: [
    { "auto.marke": "Porsche" },
    { "auto.marke": "Ferrari" }
  ]
}, { "auto.marke": 1, preis: 1, _id: 1});
print("two")
print(findOr)

const findAnd = db.mietvertrag.find({
  $and: [
    { preis: { $gt: 8000 } },
    { "auto.kennzeichen": "GR-007 007" }
  ]
},{ "auto.marke": 1, preis: 1, _id: 1});
print("three")
print(findAnd)

const findRegex2 = db.mietvertrag.find({
  "auto.kennzeichen": { $regex: /^ZH/ }
}, { "auto.marke": 1, _id: 1 });
print("four")
print(findRegex2)